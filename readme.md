...
└── pnpm-workspace.yaml
	...
	•	pnpm workspace를 이용하여 모든 레이어 패키지 링크 및 의존성 관리
	•	각 패키지별 package.json 존재, workspace: 의존성 사용*
...
"paths": {
  ...
  "@domain-user/*": ["03-domain/user/*"]
	...
	•	domain, feature, core 코드에서 공용 타입/함수 import 가능

⸻

6. .gitignore 정책
	•	node_modules, 빌드 산출물, 로그 파일 제외
	•	infra 폴더 내부: local DB, Docker volume, env 파일 제외
	•	프로젝트 안전성을 위해 infra/template, schema.prisma 등만 Git 관리
